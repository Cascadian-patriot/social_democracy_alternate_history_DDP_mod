...
window._toggleHugenbergMode = function() {
    ...
    if (!document.getElementById('hugenberg-mode-style')) {
        ...
        st.textContent =
            ...
            '  --content-bg-color:' + P + ' !important;' +   /* the panel surfaces â€” the white space */
...
window._coalitionStyledName = function(name) {
    ...
    var OVERRIDES = {
        ...
        'BÃ¼rgerblock':       '<b><span style="color: #6fa8dc;"><span style="color: #3e88b3;"><span style="color:var(--ddp-color);"><span style="color:var(--dnvp-color);">BÃ¼r</span></span></span><span style="color: #d5ac27;">ger</span><span style="color: #ffcc00;"><span style="color: #6fa8dc;"></span></span><span style="color: #ffcc00;"><span style="color: #6fa8dc;"></span><span style="color: #6fa8dc;"><span style="color:var(--dvp-color);"><span style="color:var(--ddp-color);"><span style="color:var(--dnvp-color);"><span style="color:var(--ddp-color);">blo</span></span><span style="color:#6FA8DC;"><span style="color:var(--z-color);">ck</span></span></span></span></span></span></span></b><b><span style="color: #6fa8dc;"><span style="color:var(--ddp-color);"></span></span></b>',
        'GroÃŸe BÃ¼rgerblock': '<span style="color:var(--cvp-color);">Gro<span style="color:#6FA8DC;"><span style="color:var(--cvp-color);">ÃŸe</span></span></span> <b><span style="color:var(--dnvp-color);">BÃ¼r</span><span style="color:var(--dvp-color);">ger</span><span style="color:var(--ddp-color);">blo</span><span style="color:var(--z-color);"><span style="color:#6FA8DC;">ck</span></span></b>',
        ...
        'Schwarz-WeiÃŸ-Braun Coalition': '<b><span style="color:var(--z-color);">Schwarz</span>-<span style="color:var(--dnvp-color);">WeiÃŸ</span>-<span style="color:#954B00;">Braun</span> Coalition</b>',
        'Schwarz-WeiÃŸ-Blau Coalition': '<b><span style="color:#FFCC00;">Schwarz</span>-<span style="color:var(--dvp-color);">WeiÃŸ</span>-<span style="color:#6FA8DC;">Blau</span> Coalition</b>',
        ...
        'BÃ¼rgerliche Mitte Coalition': '<b><span style="color:var(--z-color);">BÃ¼rger</span><span style="color:var(--dvp-color);">liche</span> <span style="color:var(--ddp-color);">Mitte</span> <span style="color:#6FA8DC;">Coalition</span></b>',
    ...
    var wordColor = function(w) {
        var l = w.toLowerCase().replace(/[^a-zÃ¤Ã¶Ã¼ÃŸ]/g, '');
...
window._setupGameUI = function() {
    ...
    if (dashboard) {
        ...
        if (_kr > 0 && _kn !== '' && _kn !== 'none') {
            ...
            '<div style="font-size:0.85em;color:#666;">' + (_isLehmann ? 'ParteifÃ¼hrerin' : 'ParteifÃ¼hrer') + '</div>' +
            '</div>' +
            '</div>';
        } else {
            _kaiserBlock =
        ...
        '</div>' +
            ...
            '<span><span style="display:inline-block;width:10px;height:10px;background:#06402B;border-radius:50%;"></span> <b style="color:#06402B;">VÃ¶lkisch</b></span>' +
            '</div>'
            : '') +
            '</div>';
    }
...
window._stateActions = {
    prussia: {
        ...
        actions: [
            ...
            { flag: 'prussia_flag_in_prog', label: 'Restoring the <span style="color:#000000;">Schwarz</span>-<span style="color:#888888;">WeiÃŸ</span>-<span style="color:var(--spd-color);">Rot</span>', description: 'The right of private businesses and homes to display the imperial <span style="color:#000000;">black</span>-<span style="color:#888888;">white</span>-<span style="color:var(--spd-color);">red</span> flag is being restored, an overdue cultural correction.', img: 'img/prusflag.jpg', person: 'Wolfgang von Kries', person_party: 'DNVP', duration: 4 },
...
window._updateChancellorImage = function() {
    ...
    var map = window._chancellorPortraits = {
        ...
        'Marx':       'img/marx.jpg',
        'MÃ¼ller':     'img/portraits/MÃ¼llerHermann.jpg',
        'BrÃ¼ning':    'img/bruningchancport.png',
        ...
        'Wels-MÃ¼ller':'img/portraits/WelsOtto.jpg',
...
window._getMinister = function(ministry, party) {
    ...
    return (typeof lead === 'string' && lead) ? lead : 'â€”';
};
...
window._updatePresidentImage = function() {
    ...
    var map = window._presidentPortraits = {
        ...
        'Hitler':     'img/portraits/hitler2.jpg',
        'ThÃ¤lmann':   'img/portraits/thalmann.jpg',
        'Braun':      'img/portraits/braun.jpg',
        'Rosenfeld':  'img/portraits/rosenfeld.jpg',
        'BrÃ¼ning':    'img/portraits/bruning.jpg',
        'Stegerwald': 'img/portraits/stegerwald.jpg',
        'Luther':     'img/luther1.png',
        'Westarp':    'img/portraits/westarp.png',
        'Hugenberg':  'img/portraits/AlfredHugenberg.jpg',
...
window._checkPortraits = function() {
    ...
    head.innerHTML = '<div style="font-size:26px; text-align:center; margin-bottom:4px;">Portrait Render Check</div>'
        + '<div style="text-align:center; color:#aaa; font-size:12px; margin-bottom:8px;">green border = renders &nbsp;Â·&nbsp; red = MISSING file</div>';
    ...
    overlay.appendChild(section('Presidents â€” wired', window._presidentPortraits || {}));
    overlay.appendChild(section('Chancellors â€” wired', window._chancellorPortraits || {}));
    overlay.appendChild(section('Game-settable â€” NO portrait file yet', window._portraitCandidates));
...
(function(){
  ...
  function fmtDiff(d){ var s=esc(d.k)+': '+esc(String(d.from))+' â†’ '+esc(String(d.to)); if(typeof d.from==='number'&&typeof d.to==='number'){ var dl=Math.round((d.to-d.from)*1000)/1000; s+=' <b style="color:'+(dl>=0?'#6f6':'#f66')+';">('+(dl>=0?'+':'')+dl+')</b>'; } return s; }
  function evalPred(p){
    if(!p) return null;
  ...
  function predLine(label,p){
    ...
    var body;
    if(cls.length>1){ body='<div style="margin-left:6px;">'+cls.map(function(cl){ var r=evalClause(cl); var col=r===null?'#fc6':r?'#6f6':'#f66', mk=r===null?'?':r?'âœ“':'âœ—'; return '<div style="color:#9ab;"><b style="color:'+col+';">'+mk+'</b> '+esc(stripP(cl))+'</div>'; }).join('')+'</div>'; }
...
window._resetMinistries = function() {
    ...
    for (var i = 0; i < mins.length; i++) {
        ...
        Q[mins[i] + '_minister'] = (cat.I && cat.I.length) ? cat.I[0] : 'â€”';
    }
};
...
window._distributeSpdCabinet = function(parties) {
    ...
    for (var m = 0; m < mins.length; m++) {
        ...
        var nm = window._getMinister ? window._getMinister(min, chosen) : 'â€”';
        Q[min + '_minister'] = nm || 'â€”';
    }
...
window._decorateChoices = function() {
    ...
    (function() {
        ...
        var stateMatchers = [
            ...
            { match: 'WÃ¼rttemberg',         control: 'dnvp_in_wurttemburg', timer: 'wurttemburg_action_timer' },
